...
def Abrir_paleta():
    ...
    filepath_palet=filedialog.askopenfilename(initialdir="C:\\Users\\brbru\Desktop\\Programacao\\Tkinter_py",
                                        title="Abrir Paletas:",
...
def granulometria(x,y=np.nan):
    if x in ['AND', 'BAS', 'ARS', 'CRN', 'CRV', 'CHT', 'CIM', 'DIA', 'GIP', 'GNA', 'GRN', 'HAL', 'INI', 'IGB', 'MSD', 'SNI', 'SLX', 'TQD', 'CLC']:
      return 0
    elif x in ['CAL']:
      return 1.5
    elif x in ['GRS', 'CRE']:
      ...
      return 3
    elif x in['ARG', 'AGT', 'AGN', 'AGL', 'AGC', 'AGB', 'CLU', 'FLH', 'MRG', 'ESF', 'LMT']:
      return 1
...
def Order_Lito(a):
  ...
  for i in range(len(a)):
    if a[i][0] in ['AGT', 'AGC', 'AGL', 'AGB', 'ARG', 'FLH', 'FLS', 'MRG', 'TOL', 'SLT', 'AGS', 'TOS', 'ARN', 'ARE', 'ARL', 'ARF', 'ART', 'ARC', 'CGL', 'ARO', 'BRC', 'DMT', 'CAL', 'CLC', 'CHT', 'DOL', 'CLU', 'MDS', 'WKS', 'CSI', 'PKS', 'CRE', 
'GRS', 'CRU', 'COQ', 'BLT']:
...
def litodic():
  ...
  granu=Dados()[3]
  d=intervalo(prof)
  l={prof[0]:{"Lito":[[cod_shortname(lit[0]),porcentagem(perc[0])]],"Espacamento":d[prof[0]],"Granulometria":granulometria(cod_shortname(lit[0]),granu[0])}}
  ...
  for i in range(1,len(prof)):
    if prof[i] in l:
      ...
      g=[]
      g.append([cod_shortname(lit[i]),porcentagem(perc[i]),granu[i]])
      dic={"Lito":[[cod_shortname(lit[i]),porcentagem(perc[i])]],"Espacamento":d[prof[i]],"Granulometria":granulometria(cod_shortname(lit[i]),granu[i])}
      l[prof[i]]=dic
...
def JanelaGrafico():
  ...
  def graphyc(N,L):
    ...
    prof=np.arange(N-l[N]["Espacamento"],N+1,1)
    a=(l[N]["Lito"][0][1]/100)*lim
    plt.fill_betweenx(prof,a, facecolor=str(lithologies_name[l[N]["Lito"][0][0]]["patch_property"]['color']),hatch=str(lithologies_name[l[N]["Lito"][0][0]]["patch_property"]['hatch']), alpha=alp,linewidth=0.2)
    for i in range(1,len(l[N]["Lito"])):
      if (l[N]["Lito"][i][1])!='': #TROCAR 
        b=(l[N]["Lito"][i][1]/100)*lim+a
        plt.fill_betweenx(prof,b,a,facecolor=str(lithologies_name[l[N]["Lito"][i][0]]["patch_property"]['color']),hatch=str(lithologies_name[l[N]["Lito"][i][0]]["patch_property"]['hatch']), alpha=alp,linewidth=0.2)
        a=b
  ...
  def plotGranulometria(gnm):
      ...
      for i in range(1,len(Profundidade)-1):
        plt.fill_between([-1,g[1][i]],Profundidade[i-1],Profundidade[i], facecolor=lithologies_name[g[2][i]]['patch_property']['color'],hatch=lithologies_name[g[2][i]]['patch_property']['hatch'], alpha=1)
      plt.fill_between([-1,g[1][-1]],Profundidade[-1],Profundidade[-2], facecolor=lithologies_name[g[2][-1]]['patch_property']['color'],hatch= lithologies_name[g[2][-1]]['patch_property']['hatch'],alpha=1)
      ...
      carscale=False
      if carscale!=True:
        plt.xticks([1,2,2.5,3,3.5,4,4.5,5,5.5],['Clay','Silt','\nvf','\nf','  Sand \nm','\nc','\nvc','\n   gran','Gravel'])
 ...
 # window.iconbitmap(r'sedbricon.ico')
  ...
  profmin=Label(window,text="Profundidade Mínima", font=("Arial",14)).grid(row=0, column=0)
  pmin=Entry(window, font=("Arial",14),fg="Blue")
  pmin.grid(row=0, column=1)
  profmax=Label(window,text="Profundidade Máxima", font=("Arial",14)).grid(row=1, column=0)
  pmax=Entry(window, font=("Arial",14),fg="Red")
  pmax.grid(row=1, column=1)
  graph=Button(window,text="Plot", command=plotgraph, font=("Comic Sans",15), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", width=7 )
...
def Abrir():
    def Janela():
        def Salvar():
            ...
            Posicao=[PosicaoCol(HEAD[0],data),PosicaoCol(HEAD[1],data),PosicaoCol(HEAD[2],data),PosicaoCol(HEAD[3],data)]
            #print(HEAD,Posicao)
            return HEAD
        def SairSalvar():
            HEAD=[Profvar.get(),Litvar.get(),Percvar.get(),Granvar.get()]
       ...
       # window.iconbitmap(r'sedbricon.ico')
        if filepath=='':
            ...
            proflab=Label(window,text="Profundidade", font=("Arial",15)).grid(row=1, column=0)
            Mlit=OptionMenu(window, Litvar, *TipV)
            Mlit.grid(row=2, column=1)
            #Litvar.trace_add('write')
            litolab=Label(window,text="Litologia", font=("Arial",15)).grid(row=2, column=0)
            porclab=Label(window,text="Porcentagem", font=("Arial",15)).grid(row=3, column=0)
            porclabe=OptionMenu(window, Percvar, *TipV)
            porclabe.grid(row=3, column=1)
            granlab=Label(window,text="Granulometria", font=("Arial",15)).grid(row=4, column=0)
            granlabe=OptionMenu(window, Granvar, *TipV)
            granlabe.grid(row=4, column=1)
            global HEAD
            salvar=Button(window,text="Salvar", command=Salvar, font=("Comic Sans",13), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", )
            salvar.grid(row=5, column=0,columnspan=1, pady=7, padx=7, ipadx=100)
            sair=Button(window,text="Sair", command=SairSalvar, font=("Comic Sans",13), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", )
    ...
    abr=Button(window1,text="+", command=openFile, font=("Comic Sans",13), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", width=3, )
    abr.grid(row=0, column=1,columnspan=1, pady=7, padx=7, ipadx=100)
    edt=Button(window1,text="Editar Colunas", command=Janela, font=("Comic Sans",13), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", width=10 )
    edt.grid(row=1, column=1,columnspan=1, pady=7, padx=7, ipadx=100)
    salvar=Button(window1,text="Salvar", command=SalvarDados, font=("Comic Sans",13), fg="#DCDCDC", bg="green", activeforeground="#F8F8FF",activebackground="Black", width=7, )
...
def openFile():
    global filepath
    filepath=filedialog.askopenfilename(initialdir="C:\\Users\\brbru\Desktop\\Programacao\\Tkinter_py",
                                        title="Open file:",
...
def Paletas_Criar():
    ...
    frame=Frame(windowp, bd=4,bg=backgr,highlightbackground=hlbg,highlightthickness=3)
            #place deixa as janelas autoajustaveis
    frame.place(relx=0.025,rely=0.025,relwidth=0.95, relheight=0.1)
    frame2=Frame(windowp, bd=4,bg=backgr,highlightbackground=hlbg,highlightthickness=3)
            #place deixa as janelas autoajustaveis
    ...
    estilo_notebook.configure("TNotebook", background=backgr, tabposition="n",highlightbackground=hlbg)
    if Tamanho_Mnem<=39:
        ABA=[frame2]
    else:
    ...
    paletperson=Label(frame,text="Nome da Paleta", font=("Arial",15),bg=backgr).pack(side="left",padx=7)#.grid(row=0, column=0,padx=7)
    entdic= Entry(frame, textvariable=dic_person_nome, font=("Arial",14),fg="#4B0082")
    ...
    def Salvar_Paleta():
        ...
        if None in lista_CORES:
            answer=messagebox.askyesno(title='AVISO -  Litologias sem cor!', message='Todas as litologias que estiverem vazias\n serão trocadas pela cor branca.\n\n Deseja continuar mesmo assim? ')
            if answer:
        ...
        for c in range(len(Mnemonicos)):
            dic={"short_name":Mnemonicos[c], "patch_property": {  "color": lista_CORES[c],  "hatch": opthatch[c].get()}}
    ...
    TipVar=tuple(['','/','|','-','+','/-','x','.','..','o','oo','O','-o','+o','xo','-.','-x','-.','-x','-|-','-x-','-o-','-.-'])
    ...
    def ABAS():
            ...
            for j in range(ln): #(colu//4)*4+(colu%4)
                ...
                if c<Tamanho_Mnem:
                    if i<=9 and j<=3:
                        ...
                        I=i
                        J=3*(j)
                        label = Label(AB, text=Mnemonicos[c]+":",bg=backgr,font=("Comic Sans",13))
                        label.grid(row=I+1, column=J)
                        opthatch[c].set('')
                        opt= OptionMenu(AB, opthatch[c], *TipVar)#textvariable=lista[c]
                        opt.grid(row=I+1, column=J+2)
                        colplus=Button(AB,text="+",font=("Comic Sans",13), fg="#1C1C1C", bg="#FFFFFF", activeforeground="#000000",activebackground="#FFFFFF", width=5)
        ...
        for i in range(ln+ln2):
            if i==4:
                cl=4
                AB=ABA[1]
            Corlab=Label(AB,text="Cor", font=("Arial",14),bg=backgr).grid(row=0, column=3*(i-cl)+1,padx=7, pady=7)
            Litolab=Label(AB,text="Litologia", font=("Arial",14),bg=backgr).grid(row=0, column=3*(i-cl),padx=7, pady=7)
            Hatclab=Label(AB,text="Hatch", font=("Arial",14),bg=backgr).grid(row=0, column=3*(i-cl)+2,padx=7, pady=7)
    ABAS()
    """if openPalet:
    ...
    sv=Button(frame,text="Salvar", command=Salvar_Paleta, font=("Comic Sans",15), fg="#F8F8FF", bg="#2F4F4F", activeforeground="#DCDCDC",activebackground="#5F9EA0", width=10)
    sv.pack(side="left",padx=7)#grid(row=0, column=3,padx=7)
...
abrir=Button(windowmain,text="Abrir arquivo do Poço", command=Abrir, font=("Comic Sans",15), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", )
abrir.grid(row=1, column=1,columnspan=1, pady=7, padx=7, ipadx=100)
descricao=Button(windowmain,text="Descrição", command=Descricao, font=("Comic Sans",15), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", )
descricao.grid(row=2, column=1,columnspan=1, pady=7, padx=7, ipadx=100)
graficos=Button(windowmain,text="Gráficos", command=Graficos, font=("Comic Sans",15), fg="#DCDCDC", bg="black", activeforeground="#F8F8FF",activebackground="Black", )
